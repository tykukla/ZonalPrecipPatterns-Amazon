...
## NOTE -- script has issues on PC because of characters used in plotting
...
saveDir <- 'DISTRIBUTE/ENERGY_BALANCE/LGM'
...
dfep <- readRDS(paste(saveDir, 'LGM-BrightSahel_EFcontours_and_pts.RDS', sep='/'))
dfep.contour <- dfep$contours
dfep.p <- dfep$intersects
...
myForce <- unique(dfep.contour$force_Wm2)
...
pmap <- ggplot() +
  ...
  # -- SELECTED
  # geom_path(data=dfshep.contour[which(dfep.contour$force_Wm2 %in% myLines & dfep.contour$line=="EFPM")],
  #           aes(x=lon, y=lat, group=force_Wm2, color=force_Wm2), size=1) +
  # [EAST PAC]
  geom_path(data=dfep.contour[dfep.contour$force_Wm2 %in% myLines & dfep.contour$line=="EFPM" & 
                                      dfep.contour$lon > -70 & dfep.contour$lat > -25], # cut out a weird slice
            aes(x=lon, y=lat, group=force_Wm2), color='#594839', size=0.7) +
  # [SAHEL]
  # geom_path(data=dfsh.contour[which(dfsh.contour$force_Wm2 %in% myLines & dfsh.contour$line=="EFPM")],
  #           aes(x=lon, y=lat, group=force_Wm2, color=force_Wm2), size=1) +
  # add data points
  # geom_label_repel(data=df[which(df$Depth_Profile=='Y'), ], aes(x=Lon, y=Lat, label=Sample_ID)) +
  # [east pac]
  geom_point(data=dfep.p[force_Wm2 %in% myLines], aes(x=x, y=y, size=force_Wm2), 
             shape=21, color='#594839', fill='#F2EFBD', stroke = 1.5) +
  # [sahel]
  # geom_point(data=dfsh.p[which(dfsh.p$force_Wm2 < 90),], aes(x=x, y=y, fill=abs(force_Wm2)), shape=22, color='white', size=5) +
  # scale_fill_cmocean(name=pal[pal.num], 'MSE Forcing\n(Wm2)', limits=c(0,70), start=0.6, end = 1, direction=1) +
...
saveFile <- paste(saveDir, 'LGM_BrSahel_EBM_Map.png', sep='/')
...
saveFile <- paste(saveDir, 'LGM_BrSahel_EBM_EFint.png', sep='/')
...
## ----- MAKE A FORCING MAP ---- ## 
df.all <- readRDS(paste(saveDir, 'LGM-BrightSahel_ALL.RDS', sep='/'))
# pick any two to take anomaly
df.x1 <- df.all$`Force-Wm2_0`
df.x2 <- df.all$`Force-Wm2_33`
...
p.force <- ggplot() +
  ...
  geom_text(data=mypts, aes(x=Lon, y=Lat, label="★"), size=5, family = "HiraKakuPro-W3",
            color=ptColor)  +
  scale_fill_cmocean(name="gray", guide='none', direction = -1, start = 0.7) +
  # scale_fill_cmocean(name="oxy", guide='none', direction = 1, start = 0.79, end=0.85) +
  scale_alpha_binned(guide='none', range=c(1,0.1)) +
  theme_few() +
  theme(axis.title = element_blank(), axis.text=element_blank(), axis.ticks=element_blank()) 

p.force 

saveFile <- paste(saveDir, 'LGM_BrSah-BrEur_ForceMap.png', sep='/')
